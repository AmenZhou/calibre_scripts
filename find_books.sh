...
# List of 100 public domain children's books
book_list=(
...
echo "Checking for the following books in your Calibre library (batch search):"
echo "-----------------------------------------------------------------"

# Get all titles from the Calibre library
all_calibre_titles=$(calibredb list --fields title)
...
for book_title in "${book_list[@]}"; do
    # Use grep to check if the book title exists in the list of all Calibre titles
    if echo "$all_calibre_titles" | grep -qF "$book_title"; then
        echo "Found: ${book_title}"
        found_count=$((found_count + 1))
        found_books+="- ${book_title}\n"
    else
