...
@bp.route('/read/<int:source_id>')
@login_required
def read_book(source_id):
    source = model.Source.query.get_or_404(source_id)
    ebook = source.ebook
    # Check if format is readable (EPUB or FB2)
    readable_formats = ['epub', 'fb2']
    is_readable = source.format and source.format.extension.lower() in readable_formats
    return render_template('reader.html', source=source, ebook=ebook, is_readable=is_readable)

@bp.route('/read-converted/<int:conversion_id>')
@login_required
def read_converted_book(conversion_id):
    conversion = model.Conversion.query.get_or_404(conversion_id)
    if conversion.created_by != current_user:
        abort(403, 'No Access')
    ebook = conversion.source.ebook
    # Create a pseudo-source object for the template
    class PseudoSource:
        def __init__(self, conversion):
            self.id = conversion.id
            self.format = conversion.format
            self.ebook = conversion.source.ebook
    pseudo_source = PseudoSource(conversion)
    return render_template('reader.html', source=pseudo_source, ebook=ebook, is_readable=True, is_converted=True, conversion_id=conversion_id)
