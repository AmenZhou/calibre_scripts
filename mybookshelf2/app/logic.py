...
def create_new_location(source, upload, move=False, use_symlink=False):
    """
    Create a new file location for a source.
    
    Args:
        source: Source model instance
        upload: Upload model instance or file path string
        move: If True, move the file instead of copying
        use_symlink: If True, create a symlink instead of copying/moving
                     (requires upload to be a file path string, not Upload model)
    """
    ...
    with filelock.SoftFileLock(lock_file, timeout=5):
        while os.path.exists(os.path.join(base_dir, new_location)):
            ...
            index += 1
        target_path = os.path.join(base_dir, new_location)
        if use_symlink:
            # Create symlink - source_file must be absolute path
            if os.path.exists(new_file):
                # Use absolute path for symlink target
                source_abs = os.path.abspath(new_file)
                os.symlink(source_abs, target_path)
            else:
                raise ValueError(f"Cannot create symlink: source file does not exist: {new_file}")
        elif move:
...
def stream_response(fname, mimetype, headers={}):
    """Stream a file with Range request support for EPUB.js"""
    ...
    if range_header:
        ...
        if match:
            ...
            response = Response(stream_range(), status=206, mimetype=mimetype, headers=headers)
            return response
...
def read_source(id, file_path=None):
    """Serve source file for inline reading (instead of download)
    If file_path is provided, serve a file from inside the EPUB archive
    """
    try:
        source = model.Source.query.get(id)
    except NoResultFound:
    ...
    # Only allow reading EPUB files
    if source.format and source.format.extension.lower() != 'epub':
        # For non-EPUB files, serve the file directly
        response = stream_response(fname, mimetype=source.format.mime_type,
    ...
    if not os.path.exists(fname):
        logger.error(f'Source file not found: {fname}')
    ...
    if file_path:
        ...
        try:
            with zipfile.ZipFile(fname, 'r') as z:
                # Normalize file path - EPUB files may use different path separators
                file_list = z.namelist()
                normalized_path = file_path.replace('\\', '/')
                
                # Try to find the file (case-insensitive, handle path variations)
                found_path = None
                for name in file_list:
                    if name == file_path or name == normalized_path:
                        found_path = name
                        break
                    # Try case-insensitive match
                    if name.lower() == file_path.lower() or name.lower() == normalized_path.lower():
                        found_path = name
                        break
                
                if not found_path:
                    # File not found - return 404 instead of 500
                    logger.debug(f'File {file_path} not found in EPUB {fname}')
                    abort(404, f'File {file_path} not found in EPUB')
                
                file_data = z.read(found_path)
                # Guess mimetype from extension
                mimetype = mimetype_from_file_name(found_path) or 'application/octet-stream'
                headers = {
                ...
                return Response(file_data, mimetype=mimetype, headers=headers)
        except zipfile.BadZipFile:
            logger.error(f'Invalid EPUB file: {fname}')
            abort(400, 'Invalid EPUB file')
        except Exception as e:
            logger.error(f'Error reading file {file_path} from EPUB {fname}: {e}', exc_info=True)
            # If it's a file not found error, return 404
            if 'not found' in str(e).lower() or 'No such file' in str(e):
                abort(404, f'File {file_path} not found in EPUB')
            abort(500, f'Error reading EPUB file: {str(e)}')
    
    # Serve the EPUB file itself
    headers = {
        'Content-Disposition': 'inline',
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET, HEAD, OPTIONS',
...
def read_converted(conversion_id, file_path=None):
    """Serve converted file for inline reading (instead of download)
    If file_path is provided, serve a file from inside the EPUB archive
    """
    try:
        conversion = model.Conversion.query.get(conversion_id)
    except NoResultFound:
        abort(404, 'Conversion not found')
    
    epub_fname = os.path.join(current_app.config['BOOKS_CONVERTED_DIR'], conversion.location)
    
    # Only allow reading EPUB files
    if conversion.format.extension.lower() != 'epub':
        abort(400, 'Only EPUB files can be read in the browser')
    
    # Check if EPUB file exists
    if not os.path.exists(epub_fname):
        logger.error(f'Converted file not found: {epub_fname}')
    ...
    if file_path:
        ...
        try:
            with zipfile.ZipFile(epub_fname, 'r') as z:
                # Normalize file path - EPUB files may use different path separators
                # Try exact match first, then try with different path separators
                file_list = z.namelist()
                normalized_path = file_path.replace('\\', '/')
                ...
                if not found_path:
                    # File not found - return 404 instead of 500
                    logger.debug(f'File {file_path} not found in EPUB {epub_fname}')
                    abort(404, f'File {file_path} not found in EPUB')
            ...
            logger.error(f'Invalid EPUB file: {epub_fname}')
            abort(400, 'Invalid EPUB file')
        except Exception as e:
            logger.error(f'Error reading file {file_path} from EPUB {epub_fname}: {e}', exc_info=True)
            # If it's a file not found error, return 404
            if 'not found' in str(e).lower() or 'No such file' in str(e):
                abort(404, f'File {file_path} not found in EPUB')
            abort(500, f'Error reading EPUB file: {str(e)}')
    ...
    response = stream_response(epub_fname, mimetype=conversion.format.mime_type,
                               headers=headers)
