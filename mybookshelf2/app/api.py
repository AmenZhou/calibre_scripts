...
def read(id, file_path=None):
    ...
    if request.method == 'OPTIONS':
        ...
        return response
    return logic.read_source(id, file_path)

def read_file(id, file_path):
    """Route handler for reading files from inside source EPUB"""
    # Handle OPTIONS request for CORS
    if request.method == 'OPTIONS':
        response = Response('')
        response.headers['Access-Control-Allow-Origin'] = '*'
        ...
        response.headers['Access-Control-Allow-Headers'] = 'Range, Content-Type'
        return response
    return read(id, file_path)


def read_converted(id, file_path=None):
    conversion = model.Conversion.query.get_or_404(id)
    if conversion.created_by != current_user:
        abort(403, 'No Access')
    return logic.read_converted(id, file_path)

def read_converted_file(id, file_path):
    """Route handler for reading files from inside converted EPUB"""
    return read_converted(id, file_path)
...
# Route for internal EPUB files from source - EPUB.js appends paths to the base URL
bp.add_url_rule('/read/<int:id>/<path:file_path>', 'read_file', read_file, methods=['GET'])
add_url(read_converted, '/read-converted/<int:id>')
# Route for internal EPUB files from converted - EPUB.js appends paths to the base URL
bp.add_url_rule('/read-converted/<int:id>/<path:file_path>', 'read_converted_file', read_converted_file, methods=['GET'])
